...
export class GoblinQuestActorSheet extends foundry.appv1.sheets.ActorSheet {
    ...
    async _onRollButtonClick(event) {
        ...
        // Calculate dynamic dice size to fit in the fixed container
        // Container is 140px high with 10px padding, so usable height is 120px.
        // We assume a conservative average width for the chat message.
        const areaW = 230; // Reduced width safety margin
        const areaH = 100; // Reduced height to ensure vertical space remains
        const gap = 4;
        const count = displayedResults.length;
        
        // Calculate max square size: sqrt(Area / (count * inefficiency_factor)).
        // The inefficiency factor (1.5) accounts for imperfect packing by flexbox.
        let dieSize = Math.floor(Math.sqrt((areaW * areaH) / (count * 1.5))) - gap;
        // Clamp size (Max 40px, Min 10px)
        dieSize = Math.min(40, Math.max(10, dieSize));
        const fontSize = Math.floor(dieSize * 0.6);
        ...
        const diceBoxesHtml = displayedResults.map(result => {
            ...
            return `<div class="${cssClass}" style="width: ${dieSize}px; height: ${dieSize}px; font-size: ${fontSize}px;">${displayValue}</div>`;
        }).join('');
        ...
        const flavorText = `
            <div class="goblin-chat-message">
                <div class="goblin-header">
                    <img class="goblin-portrait" src="${activeGoblinImg}" title="${activeGoblinName}" />
                    <div class="goblin-title">
                        <span class="goblin-name">${activeGoblinName}</span>
                        <span class="action-text">lanza ${actualDiceToRoll} dados ${diceModifier !== 0 ? `[${formattedModifier}]` : ''}</span>
                    </div>
                </div>
                <div class="dice-container">
                    ${diceBoxesHtml}
                </div>
                <div class="roll-summary-container">
                    <div class="summary-box success"><span class="label">Éxitos</span><span class="value">${successes}</span></div>
                    <div class="summary-box wound"><span class="label">Heridas</span><span class="value">${wounds}</span></div>
                </div>
            </div>
        `;
